...
typedef std::function<hipError_t(
    ...
    hipStream_t)>  // stream: stream to execute in.
    FloatSpmmFn;
...
hipError_t CudaSpmmBiasRelu(
    ...
    float* __restrict__ output_matrix, hipStream_t stream) {
...
hipError_t CudaSpmmBiasRelu(
    ...
    half2* __restrict__ output_matrix, hipStream_t stream) {
...
#define INSTANTIATE_TILED_FLOAT(fn, stype, dtype, mt, kt, nt, bs)           \
  template hipError_t fn<SpmmConfig<float, stype, dtype, mt, kt, nt, bs>>( \
      int, int, int, int, const int*, const float*, const int*, const int*, \
      const float*, const float*, float*, hipStream_t);
...
#define INSTANTIATE_TILED_HALF(fn, stype, dtype, mt, kt, nt, bs)               \
  template hipError_t fn<SpmmConfig<half2, stype, dtype, mt, kt, nt, bs>>(    \
      int, int, int, int, const int*, const half2*, const int*, const short2*, \
      const half2*, const float*, half2*, hipStream_t);
