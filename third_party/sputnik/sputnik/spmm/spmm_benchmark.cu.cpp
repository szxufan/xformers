...
void BM_CublasGemm_ColumnMajor(benchmark::State& state) {
  ...
  hipblasDatatype_t data_type = HIPBLAS_R_32F;
  hipblasDatatype_t compute_type = HIPBLAS_R_32F;
  hipblasGemmAlgo_t gemm_algorithm = HIPBLAS_GEMM_DEFAULT;

  int batch_size = 10;
  while (state.KeepRunningBatch(batch_size)) {
    for (int i = 0; i < batch_size; ++i) {
      // Run the cublas kernel.
      CUBLAS_CALL(hipblasGemmEx(
          handle, HIPBLAS_OP_N, HIPBLAS_OP_N, m, n, k, &alpha, lhs_gpu.Values(),
...
hipblasStatus_t RowMajorGemm(hipblasHandle_t handle, bool trans_a,
  ...
  float alpha = 1.0, beta = 0.0;
  return hipblasGemmEx(handle, transpose_b, transpose_a, m, n, k, &alpha,
                      b.Values(), HIPBLAS_R_32F, ldb, a.Values(), HIPBLAS_R_32F, lda,
                      &beta, c->Values(), HIPBLAS_R_32F, c->Columns(), HIPBLAS_R_32F,
